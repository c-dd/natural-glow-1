...
import { useProducts, useAuth, CATEGORIES } from '@/lib/store';

const chipStyle = (a) =>
  "font:500 11px 'Manrope',sans-serif;padding:8px 16px;border-radius:999px;cursor:pointer;white-space:nowrap;transition:border-color .2s ease;" +
  (a
    ? 'color:#FFFFFF;background:#9EAF8B;border:1px solid #2E3627;'
    : 'color:#4A5540;background:#fff;border:1px solid rgba(45,53,39,.14);');

// Chips are derived from the shared CATEGORIES (7) so the filter can never
// drift from the catalog. The `key` for a category chip IS the category name.
          ...
          <div style={{ flex: 1, minHeight: 0 }}>
            ...
            <Box as="div" style="padding:7vh 5vw 9vh;animation:ngRise .5s cubic-bezier(.2,.7,.2,1)">
              <Box as="div" style="max-width:900px;margin:0 auto">
                ...
                <Box as="div" className="ng-chips" style="display:flex;flex-wrap:wrap;gap:10px;margin:30px 0 28px">
                  {CHIPS.map((c) => (
