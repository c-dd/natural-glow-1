        ...
        <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#FFDFE0', color: '#2E3627' }}>
          ...
          <div style={{ flex: 1, minHeight: 0 }}>
            ...
            <Box as="div" style="animation:ngRise .5s cubic-bezier(.2,.7,.2,1)">
              <Box as="div" style="padding:8vh 5vw 6vh;border-bottom:1px solid rgba(45,53,39,.09)">
                <Box as="div" style="max-width:900px;margin:0 auto">
                  <Box as="div" style="font:500 10px 'Space Mono',monospace;letter-spacing:.24em;text-transform:uppercase;color:#5A6B4B;margin-bottom:16px">Quality &amp; methods</Box>
                  ...
                  <Box as="p" style="margin:18px 0 0;font:400 15px/1.75 'Manrope',sans-serif;color:#4A5540;text-wrap:pretty">We treat every batch as a reference material. Every lot is characterized analytically and reported on a lot-specific Certificate of Analysis — so what's on the label is what's in the vial.</Box>
                </Box>
              </Box>
              <Box as="div" style="padding:7vh 5vw 9vh">
                <Box as="div" className="ng-grid3" style="display:grid;grid-template-columns:repeat(3,1fr);gap:22px;max-width:900px;margin:0 auto">
                  {METHODS.map((m) => (
                    <Box key={m.t} as="div" style="background:#fff;border:1px solid rgba(45,53,39,.1);border-radius:14px;padding:28px;transition:transform .25s ease, box-shadow .25s ease" hover="transform:translateY(-3px);box-shadow:0 20px 44px -26px rgba(45,53,39,.3)">
                      <Box as="div" style="font:600 25px 'Spectral',serif;color:#2E3627">{m.t}</Box>
                      <Box as="div" style="font:500 9px 'Space Mono',monospace;letter-spacing:.14em;text-transform:uppercase;color:#5A6B4B;margin:10px 0 14px">{m.tag}</Box>
                      <Box as="p" style="margin:0;font:400 13px/1.7 'Manrope',sans-serif;color:#4A5540">{m.d}</Box>
                    </Box>
                  ))}
                </Box>
