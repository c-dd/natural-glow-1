...
// =====================================================================
// Natural Glow (Direction 1) — client data layer.
// Products, accounts, and orders are SERVER truth (Netlify Functions +
// Blobs), reached through apiFetch below. Only three things still live in
// localStorage: the RUO gate pass (`ng_ruo_gate`), an optimistic session
// hint that is always reconciled against the API (`ng2-auth-hint`), and the
// dashboard cart (`ng1-cart-v2`). Every other key the earlier single-file
// build kept in localStorage is retired and swept once per browser on load
// (see sweepRetiredKeys). SSR-safe hooks seed a constant, then hydrate in
// effect.
//
// The catalog shape comes from the shared, dependency-free data module
// lib/catalog-data.mjs (the contract with the serverless seed). COA is
// OPTIONAL: consumables carry purity:null / lot:null and every surface
// degrades gracefully (see hasCOA + decorate).
// =====================================================================
...
// ---------- one-time sweep of retired localStorage keys ----------
// The early single-file stores (v1) and the interim product/order/inventory
// stores (the `-v2` bump) are all dead: that data is now server truth (Netlify
// Functions + Blobs). Remove them once per browser so a returning visitor's
// stale ids/orders can't linger or resurface. LIVE keys are intentionally NOT
// in this list: `ng_ruo_gate`, `ng2-auth-hint`, `ng1-cart-v2`.
const RETIRED_KEYS = [
  'ng_account',            // account is now the session user profile (server)
  'ng1-auth', 'ng1-auth-v2', // superseded by the confirmed session + ng2-auth-hint
  'ng1-cart',              // v1 cart (v2 = ng1-cart-v2 is LIVE, kept)
  'ng1-inventory', 'ng1-inventory-v2',
  'ng1-orders', 'ng1-orders-v2',
  'ng1-order-seq', 'ng1-order-seq-v2',
  'ng1-products-extra', 'ng1-products-extra-v2',
];
const SWEEP_SENTINEL = 'ng-sweep-v6';
function sweepRetiredKeys() {
  if (!isBrowser()) return;
  try {
    if (localStorage.getItem(SWEEP_SENTINEL)) return; // already swept this browser
    RETIRED_KEYS.forEach((k) => { try { localStorage.removeItem(k); } catch {} });
    localStorage.setItem(SWEEP_SENTINEL, '1');
...
// v2: v1 carts held retired v1 product ids (v1 key swept by sweepRetiredKeys).
